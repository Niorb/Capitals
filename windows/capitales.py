	...
	if not blank:
		...
		if (secondCounter==5 and counter==3 and len(secondRightList)!=0):
			...
			if(answer==el["capital"].strip()):
				print("Right answer ! the continent is: "+Fore.CYAN +el["continent"]+Style.RESET_ALL)
				print("Memo:"+ Fore.RED+el["memo"]+"\n"+Style.RESET_ALL)
				secondRightList.pop(question)
				continue
			else:
				print("\nFalse, the answer is: "+Fore.GREEN,el["capital"],Style.RESET_ALL+"in: ",Fore.CYAN,el["continent"]+Style.RESET_ALL)
